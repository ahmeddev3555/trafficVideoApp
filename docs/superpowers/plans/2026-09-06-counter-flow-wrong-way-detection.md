...
- Widened stationary-approach eligibility: `OneWay` and **all** `DirectionResolution.Unknown` reasons. Counter-flow eligibility (**revised by the whole-branch review, 2026-09-06**): `OneWay` and `Unknown(DIVIDED_CARRIAGEWAY)` at the base gate (`counter_flow_min_coherence` 0.60 / `counter_flow_min_with_flow` 5); every **other** `Unknown` reason additionally requires `flow_coherence ≥ counter_flow_strong_coherence` (0.85) AND `forwardStream ≥ counter_flow_strong_min_with_flow` (8) — `NO_ONEWAY_TAG` / `AMBIGUOUS_NEAREST_STREET` are two-way in OSM, so a lone counter-flower is only trusted there on an overwhelmingly one-directional scene. `NotFound` / `LookupFailed` / `TwoWay` stay ineligible for both.
