...
13. While recording, face the phone toward a known compass direction (use a second phone's compass app, or a known landmark/road orientation, as ground truth). Confirm the pin's head points that same real-world direction on the map - turning the phone to the right must swing the pin to the right (clockwise), not left.
14. While recording, rotate through all four cardinal-ish directions (roughly N, E, S, W) and confirm the pin stays visually legible and inside the 64dp map box at each orientation - it should not extend past the map's edges or get clipped.
15. Trigger a camera error (e.g. deny/revoke camera permission mid-session, or another repeatable error path) while a GPS fix is present, and confirm the resulting error message is fully readable and not meaningfully obscured by the map in the bottom-right corner.
16. Record a full multi-minute clip with the map visible throughout and confirm no dropped frames, stutter, or recording-finalize errors - this is the first feature where map tile loading/rendering happens concurrently with active video capture.
